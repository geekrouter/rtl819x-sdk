...
int find_uid_for_user( char *user )
{
        ...
        while ( !feof( fp ) ) {
                if ( fgets( t, 255, fp ) ) {
                        ...
                        p1 = strchr( t, ':' );
                        if ( p1 && ( p1 - t == strlen( user ) )
                             && ( strncmp( t, user, strlen( user ) ) ) ==
                             0 ) {
                                p1 = strchr( t, ':' );
                                if ( !p1 )
                                        goto out;
...
int main( int argc, char **argv )
{
        ...
        s += snprintf( s, 256 - ( s - smbpasswd_line ),
                       "%s:%u:XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX:", argv[1],
                       uid );
        for ( g = 0; g < 16; g++ )
                s += snprintf( s, 256 - ( s - smbpasswd_line ), "%02X",
                               new_nt_p16[g] );
        snprintf( s, 256 - ( s - smbpasswd_line ),
                  ":[U          ]:LCT-00000001:\n" );
