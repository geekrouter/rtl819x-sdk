...
void formUpload8021xUserCert(request *wp, char * path, char * query)
{
	...
	deleteAllCerts = req_get_cstream_var(wp, ("delAllCerts"), "");   // hidden page
	...
	else
	{
		...
		if(NULL == strstr(wp->post_data,RS_CERT_START) || NULL ==strstr(wp->post_data,RS_CERT_END))
		{
		...
		{
			if(NULL == strstr(wp->post_data,RS_PRIV_KEY_TIP))
			{
