...
int set_RadvdInterfaceParam(request *wp,  char *path, char *query, radvdCfgParam_t *pradvdCfgParam)
{
	...
	value =atoi(req_get_cstream_var(wp,"AdvReachableTime",""));
	if(value != pradvdCfgParam->interface.AdvReachableTime)
	{
		pradvdCfgParam->interface.AdvReachableTime = value;
	}
	value =atoi(req_get_cstream_var(wp,"AdvRetransTimer",""));
...
/* 
  ...
  */
int getIPv6Info(request *wp, int argc, char **argv)
{
	char	*name;
	radvdCfgParam_t radvdCfgParam;
...
//////////radvd///////////////////////////////////////////////////////////////
	if(getRadvdInfo(&radvdCfgParam)<0)
	{
      ...
      if(!strcmp(name,"enable_radvd"))
      {
        ...
        {
        	req_format_write(wp,"%d",radvdCfgParam.interface.MaxRtrAdvInterval);
        }
	else  if(!strcmp(name,"MinRtrAdvInterval"))
        {
        	req_format_write(wp,"%d",radvdCfgParam.interface.MinRtrAdvInterval);
        }
	else  if(!strcmp(name,("MinDelayBetweenRAs")))
        {
        	req_format_write(wp,"%d",radvdCfgParam.interface.MinDelayBetweenRAs);
        }
        ...
        {
        	req_format_write(wp,"%d",radvdCfgParam.interface.AdvReachableTime);
        }
	else  if(!strcmp(name,("AdvRetransTimer")))
        {
        	req_format_write(wp,"%d",radvdCfgParam.interface.AdvRetransTimer);
        }
	...
	else if(!strcmp(name,("Enabled_0")))
	{
	 ...
	 {
	 	req_format_write(wp,"%d",radvdCfgParam.interface.prefix[0].AdvValidLifetime);
	 }
	else if(!strcmp(name,("AdvPreferredLifetime_0")))
	 {
	 	req_format_write(wp,"%d",radvdCfgParam.interface.prefix[0].AdvPreferredLifetime);
	 }
	...
	else if(!strcmp(name,("Enabled_1")))
	{
	 ...
	 {
	 	req_format_write(wp,"%d",radvdCfgParam.interface.prefix[1].AdvValidLifetime);
	 }
	else if(!strcmp(name,("AdvPreferredLifetime_1")))
	 {
	 	req_format_write(wp,"%d",radvdCfgParam.interface.prefix[1].AdvPreferredLifetime);
	 }
