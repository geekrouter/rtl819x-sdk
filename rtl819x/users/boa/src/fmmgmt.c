...
void formPocketWizardGW(request *wp, char *path, char *query)
{
	char *tmpStr, *strVal;
	char tmpBuf[100];
	char varName[20];
	...
	strVal = req_get_cstream_var(wp, "mode0", "");
	val = atoi(strVal);
	apmib_set( MIB_WLAN_MODE, (void *)&val);

	strVal = req_get_cstream_var(wp, "pocket_ssid", "");
	...
	run_init_script("all");
#endif
	tmpStr = req_get_cstream_var(wp, ("hiwanType"), "");
	REBOOT_WAIT("/wizard.htm");
